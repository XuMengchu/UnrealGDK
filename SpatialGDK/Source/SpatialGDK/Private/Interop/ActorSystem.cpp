...
void ActorSystem::ApplyComponentAdd(const Worker_EntityId EntityId, const Worker_ComponentId ComponentId, Schema_ComponentData* Data)
{
	...
	case SpatialConstants::NET_OWNING_CLIENT_WORKER_FIELD_ID:
...
void ActorSystem::AuthorityLost(const Worker_EntityId EntityId, const Worker_ComponentId ComponentId)
{
	HandleActorAuthority(EntityId, ComponentId, WORKER_AUTHORITY_AUTHORITATIVE);
}

void ActorSystem::AuthorityGained(Worker_EntityId EntityId, Worker_ComponentId ComponentId)
{
	...
	if (NetDriver->SpatialDebugger != nullptr && ComponentId == SpatialConstants::AUTHORITY_INTENT_COMPONENT_ID)
	{
...
void ActorSystem::ComponentUpdated(const Worker_EntityId EntityId, const Worker_ComponentId ComponentId, Schema_ComponentUpdate* Update)
{
	...
	TOptional<Trace_SpanId> CauseSpanId = NetDriver->Connection->GetEventTracer()->GetSpanId(EntityComponentId(EntityId, ComponentId));
	if (CauseSpanId.IsSet())
	{
		TOptional<Trace_SpanId> SpanId = NetDriver->Connection->GetEventTracer()->CreateSpan(&CauseSpanId.GetValue(), 1);
		NetDriver->Connection->GetEventTracer()->TraceEvent(
			FSpatialTraceEventBuilder::CreateComponentUpdate(Channel->Actor, TargetObject, EntityId, ComponentId), SpanId);
	}
...
void ActorSystem::ReceiveActor(Worker_EntityId EntityId)
{
	...
	// const FString& ClassPath = ActorComponents.Metadata->ClassPath;
