	...
	for (const auto& Message : Messages)
	{
		...
		case WORKER_OP_TYPE_CREATE_ENTITY_RESPONSE:
		{
			...
			if (Op.request_id == RoutingWorkerRequest)
			{
				...
				RoutingWorkerRequest = Connection->SendCommandRequest(RoutingWorkerSystemEntityId, &ClaimRequest,
																	  SpatialConstants::WORKER_CLAIM_PARTITION_COMMAND_ID, {});
			}
			break;
		}
...
{
	RoutingWorkerRequest = Connection->SendReserveEntityIdsRequest(2);
}

void SpatialRoutingSystem::CreateRoutingPartition(SpatialOSWorkerInterface* Connection)
{
	AuthorityDelegationMap Map;
	Map.Add(SpatialConstants::WELL_KNOWN_COMPONENT_SET_ID, RoutingPartition);
	...
	RoutingWorkerRequest = Connection->SendCreateEntityRequest(Components, &RoutingPartition);
}

void SpatialRoutingSystem::Destroy(SpatialOSWorkerInterface* Connection)
{
	Connection->SendDeleteEntityRequest(RoutingPartition);
}
