...
void USpatialNetDriver::QueryRoutingPartition()
{
	...
	if (RoutingWorkerId == 0)
	{
		...
		Worker_RequestId RequestID = Connection->SendEntityQueryRequest(&Query);
...
void USpatialNetDriver::TryFinishStartup()
{
	...
	if (IsServer())
	{
		...
		if (WorkerType == SpatialConstants::RoutingWorkerType)
		{
			...
			RoutingSystem = new SpatialGDK::SpatialRoutingSystem(newView, Connection->GetWorkerSystemEntityId());
		...
		if (WorkerType == SpatialConstants::DefaultServerWorkerType)
		{
			...
			else
			{
				...
				{
					LoadBalanceEnforcer->SetRoutingPartition(RoutingPartition);
				}
